...
for(const file of fs.readdirSync('content').filter(f=>f.endsWith('.md'))){
 ...
 for(const k of ['title','date','category','summary'])if(!meta[k])throw Error(file+' needs '+k);
 const slug=file.slice(0,-3);if(!/^[a-z0-9-]+$/.test(slug))throw Error('Use English lowercase slugs');
 const toc=[];let n=0;
